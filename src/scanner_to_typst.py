...
class Finding:
    ...
    ai_drafted: list = field(default_factory=list)  # v0.6: which fields got LLM prose
...
def main(argv: list | None = None) -> int:
    ...
    p.add_argument("--draft-prose", action="store_true",
                   help="Use Anthropic API to draft business-impact + evidence-intro "
                        "fields. Requires ANTHROPIC_API_KEY. Marks drafted fields.")
    ...
    p.add_argument("--industry", default="",
                   help="Client industry (e.g. 'higher education') for --draft-prose context")
    ...
    if args.draft_prose:
        ...
        print(f"[+] Drafting prose via {args.llm_model}", file=sys.stderr)
        for f in findings:
            d = {
                "id": f.id, "title": f.title, "severity": f.severity,
                "cvss_score": f.cvss_score, "description": f.description,
                "business_impact": f.business_impact, "evidence": f.evidence,
            ...
            for fld in d.get("_ai_drafted", []):
                if fld not in f.ai_drafted:
                    f.ai_drafted.append(fld)
